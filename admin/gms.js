...
const API="https://juniorgame-stripe.onrender.com", nf=new Intl.NumberFormat("es-MX"), df=new Intl.DateTimeFormat("es-MX",{dateStyle:"medium",timeStyle:"short"});
const $=id=>document.getElementById(id), safe=v=>String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[c])), num=v=>Number.isFinite(Number(v))?Number(v):0;
let users=[],audit=[];
const name=d=>d.nombre||d.customName||d.name||d.displayName||"Usuario sin nombre", email=d=>d.email||"Sin correo", jf=d=>d.playerId||d.jfId||d.JF_ID||d["JF-ID"]||d.shortId||"Sin ID JF";
const date=v=>{try{const x=v?.toDate?v.toDate():v?new Date(v):null;return x&&!isNaN(x)?x:null}catch{return null}};
function status(d){const until=date(d.suspendedUntil);if(d.banned||d.disabled)return "banned";if(d.suspended&&(!until||until>new Date()))return "suspended";return "active"}
...
function drafts(){try{return JSON.parse(localStorage.getItem("jg.gms.events")||"[]")}catch{return[]}}
function renderEvents(){const a=drafts();$("gmsEventDrafts").innerHTML=a.length?a.map((x,i)=>`<div class="data-row"><div><strong>${safe(x.name)}</strong><span>${safe(x.type)} · ${safe(x.reward||"Sin recompensa")}</span><small>${x.start?safe(df.format(new Date(x.start))):"Sin fecha"}</small></div><button class="gms-event-delete" data-event-delete="${i}" type="button">×</button></div>`).join(""):'<div class="empty-inline">No hay eventos preparados.</div>';document.querySelectorAll("[data-event-delete]").forEach(b=>b.addEventListener("click",()=>{const a=drafts();a.splice(Number(b.dataset.eventDelete),1);localStorage.setItem("jg.gms.events",JSON.stringify(a));renderEvents()}))}
async function loadAudit(){try{const p=await api("/admin/audit");audit=Array.isArray(p.items)?p.items:[];renderAudit()}catch(e){$("gmsAuditList").innerHTML=`<div class="empty-inline">${safe(e.message)}</div>`}}
function renderAudit(){const q=($("gmsAuditSearch")?.value||"").toLowerCase(),filt=$("gmsAuditFilter")?.value||"all";const a=audit.filter(x=>(filt==="all"||x.action===filt||(filt==="moderation"&&!['adjustBalance','inventory'].includes(x.action)))&&(!q||[x.actionLabel,x.targetName,x.targetUid,x.reason,x.adminEmail].some(v=>String(v||"").toLowerCase().includes(q))));$("gmsAuditList").innerHTML=a.length?a.map(x=>`<div class="data-row"><div><strong>${safe(x.actionLabel||x.action)}</strong><span>${safe(x.targetName||x.targetUid)} · ${safe(x.reason||"Sin motivo")}</span><small>${safe(x.adminEmail||x.adminUid||"")}</small></div><div class="value">${x.createdAt?safe(df.format(new Date(x.createdAt))):"Pendiente"}</div></div>`).join(""):'<div class="empty-inline">No hay acciones para este filtro.</div>'}
function renderAll(){renderPlayers();renderInventory();renderModeration();renderEvents()}
$("gmsPlayerSearch")?.addEventListener("input",renderPlayers);$("gmsPlayerState")?.addEventListener("change",renderPlayers);$("gmsRefreshPlayers")?.addEventListener("click",()=>loadUsers(true));$("gmsSaveEvent")?.addEventListener("click",()=>{const name=$("gmsEventName").value.trim();if(!name){$("gmsEventName").focus();return}const a=drafts();a.unshift({name,type:$("gmsEventType").value,reward:$("gmsEventReward").value.trim(),start:$("gmsEventStart").value,notes:$("gmsEventNotes").value.trim(),createdAt:Date.now()});localStorage.setItem("jg.gms.events",JSON.stringify(a.slice(0,50)));["gmsEventName","gmsEventReward","gmsEventStart","gmsEventNotes"].forEach(id=>$(id).value="");renderEvents()});$("gmsRefreshAudit")?.addEventListener("click",loadAudit);$("gmsAuditSearch")?.addEventListener("input",renderAudit);$("gmsAuditFilter")?.addEventListener("change",renderAudit);
window.addEventListener("juniorgame:admin-ready",()=>loadUsers().catch(console.error),{once:true});window.addEventListener("juniorgame:view-change",e=>{if(["jugadores","inventario","moderacion"].includes(e.detail?.view))loadUsers().catch(console.error);if(e.detail?.view==="auditoria")loadAudit()});
